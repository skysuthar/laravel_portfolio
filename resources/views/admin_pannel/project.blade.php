			    ...
			    <tbody>
			      ...
			      <tr class="text-center">
			        ...
			        <td>{{$project->project_name}}</td>
			   		<td><img src='{{asset("root/projectImages/$project->project_image")}}' height="60px" width="60px"></td>
			   		<td>@php 
	                  echo strlen($project->project_link) >= 30 ? substr($project->project_link, 0,30):$project->project_link;
	              @endphp
	         	</td>
			   		<td>@php 
	                  echo strlen($project->project_description) >= 100 ? substr($project->project_description, 0,100):$project->project_description;
	              @endphp
	          </td>
			   		<td>{{$project->project_status}}</td>
			   		<td>
	                    <div class="leave-action text-start d-flex">
	                        <a href="{{ route('editProject',$project->id)}}" class="eye">
	                            <i class="far fa-edit  text-primary" style="width: 25px; height: 25px; border-radius: 50%; padding-top:5px; padding-left: 5px; margin-right: 10px; background-color: #fff; box-shadow: 0px 4px 10px #888; font-size: 16px;"></i>
	                        </a>
	                        <a href="javascript:void(0);" data-target="#accept" data-toggle="modal" class="approve ml-1 delete" data-id="{{$project->id}}">
